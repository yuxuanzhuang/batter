...
import sys as sys
import batter.lib.scripts as scripts
import filecmp
...
def build_equil(pose, celp_st, mol,
    ...
    if '/' in H1:
        h1_atom = H1.split('@')[1].split('/')[0]
        h2_atom = H2.split('@')[1].split('/')[0]
        h3_atom = H3.split('@')[1].split('/')[0]

        h1_chain = H1.split('@')[1].split('/')[1]
        h2_chain = H2.split('@')[1].split('/')[1]
        h3_chain = H3.split('@')[1].split('/')[1]
    else:
        h1_atom = H1.split('@')[1]
        h2_atom = H2.split('@')[1]
        h3_atom = H3.split('@')[1]

        protein_chain = u_pdb4amber.atoms.residues[0].chainID
        h1_chain = protein_chain
        h2_chain = protein_chain
        h3_chain = protein_chain
    
    h1_entry = (renum_data
            .query('old_resid == @h1_resid')
            .query('old_chain == @h1_chain')
    )
    h2_entry = (renum_data
            .query('old_resid == @h2_resid')
            .query('old_chain == @h2_chain')
    )
    h3_entry = (renum_data
            .query('old_resid == @h3_resid')
            .query('old_chain == @h3_chain')
    )
    # check not empty
    ...
    p1_vmd = f'{p1_resid} and chain {h1_chain}'

#    p1_resid = str(int(h1_resid) - int(first_res_vmd) + 2)
#    p2_resid = str(int(h2_resid) - int(first_res_vmd) + 2)
#    p3_resid = str(int(h3_resid) - int(first_res_vmd) + 2)

#    p1_vmd = str(int(h1_resid) - int(first_res_vmd) + 1)
#    p1_vmd = f'{p1_vmd} and chain {h1_chain}'

    P1 = f':{p1_resid}@{h1_atom}/{h1_chain}'
    P2 = f':{p2_resid}@{h2_atom}/{h2_chain}'
    P3 = f':{p3_resid}@{h3_atom}/{h3_chain}'
...
def build_dec(fwin, hmr, mol,
    ...
    if (not os.path.exists(f'../{build_file_path}')) or (dec_method == 'sdr' and win == 0) or (dec_method == 'exchange' and win == 0):
        ...
        if '/' in P1:
            p1_atom = P1.split('@')[1].split('/')[0]
            p1_chain = P1.split('@')[1].split('/')[1]
        else:
            p1_atom = P1.split('@')[1]
            p1_chain = None
        ...
        p1_vmd = p1_resid
        if p1_chain:
            p1_vmd = f'{p1_resid} and chain {p1_chain}'
