...
if [[ $only_eq -eq 1 ]]; then
    ...
    for i in $(seq 0 $((24 - 1))); do
        win_folder=$(printf "../z%02d" $i)
        if [[ -s $win_folder/mini.rst7 ]]; then
            ...
            cd $win_folder
            pmemd.cuda_DPFP -O -i mini.in -p $PRMTOP -c ../z-1/eqnpt04.rst7 -o mini.in.out -r mini.in.rst7 -x mini.in.nc -ref ../z-1/eqnpt04.rst7 >> "$log_file" 2>&1
            ...
            if ! check_min_energy "mini.in.out" -1000; then
                ...
                if [[ $SLURM_JOB_CPUS_PER_NODE -gt 1 ]]; then
                    mpirun --oversubscribe -np $SLURM_JOB_CPUS_PER_NODE pmemd.MPI -O -i mini.in -p $PRMTOP -c ../z-1/eqnpt04.rst7 -o mini.in.out -r mini.in.rst7 -x mini.in.nc -ref ../z-1/eqnpt04.rst7 >> "$log_file" 2>&1
                else
                    pmemd -O -i mini.in -p $PRMTOP -c ../z-1/eqnpt04.rst7 -o mini.in.out -r mini.in.rst7 -x mini.in.nc -ref ../z-1/eqnpt04.rst7 >> "$log_file" 2>&1
                fi
                check_sim_failure "Minimization for window $i" "$log_file"
                if ! check_min_energy "mini.in.out" -1000; then
