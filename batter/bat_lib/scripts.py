...
def write_tleap(mol, molr, comp, water_model, water_box, buff, buffer_x, buffer_y, other_mol, tleap_remove=None):
    ...
    tmp_file.write('solvatebox model ' + water_box + ' {'+ str(buffer_x) +' '+ str(buffer_y) +' '+ str(buff) + '} 1.5 \n')
...
def box_volume():
    ...
    for line in f:
        ...
        if "Density" in line:
            ...
            # it should be 6, but it was 5 in the original code
            # -2 should be more error-proof
            ratio = float(float(splitdata[-2])/dens)
    f.close()
