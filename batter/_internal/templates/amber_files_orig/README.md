...
| --- | --- | --- |
| Equilibration (`prepare_equil`) | `mini.in`, `eqnvt.in`, `eqnpt0.in`, `eqnpt0-water.in`, `eqnpt.in`, `eqnpt-water.in`, `mdin-equil` | `batter/_internal/ops/sim_files.py` copies these into each equilibration directory. The `*-water` variants are chosen when `sim.membrane_simulation` is `False`. |
...
## Templates not referenced in `_internal`

All unused legacy templates have been removed as of this change. Use `rg` to
confirm whether a new template is referenced before deleting it in the future.
