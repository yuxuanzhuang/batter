...
def copy_simulation_dir(source: Path, dest: Path, sim: SimulationConfig) -> None:
    ...
    if str(getattr(sim, "hmr", "no")).lower() == "yes":
    ...
    # propagate anchors.json if available
    try:
        anchors = load_anchors(source.parent)  # saved at working_dir
        save_anchors(dest.parent, anchors)
    except Exception:
        pass