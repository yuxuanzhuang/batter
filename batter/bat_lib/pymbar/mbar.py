...
from batter.bat_lib.pymbar import mbar_solvers
from batter.bat_lib.pymbar.utils import kln_to_kn, kn_to_n, ParameterError, DataError, logsumexp, check_w_normalized
        ...
        if (method == 'zeros'):
            ...
            for index in range(0, np.size(initialization_order) - 1):
                ...
                if (len(w_F) > 0 and len(w_R) > 0):
                    ...
                    # kickstart NR.
                    import lib.pymbar.bar
                    self.f_k[l] = self.f_k[k] + lib.pymbar.bar.BAR(
