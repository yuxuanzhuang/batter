...
def _cpptraj_export_rep(rep_idx: int, prmtop: str, n_eq: int, workdir: Path) -> None:
    """
    ...
    n_eq : int
        Number of equilibration segments named md-XX.nc. We will skip the *first* segment
        by design and read md-01.nc ... md-(n_eq-1).nc.
    workdir : Path
        Directory containing the md-XX.nc trajectories; rep.in will be written here.
    """
    if n_eq < 2:
        raise ValueError("n_eq must be ≥ 2 so that at least one md-XX.nc is read after skipping the first.")

    if rep_idx < 0:
        raise ValueError("rep_idx must be ≥ 0 (0-based)")

    # Build cpptraj input
    lines: List[str] = [f"parm {prmtop}"]
    for i in range(1, n_eq):  # skip the first equil traj (assumes md-00.nc exists and is skipped)
    ...
    run_with_log("cpptraj -i rep.in", working_dir=workdir)
...
def equil_analysis_handler(step: Step, system: SimSystem, params: Dict[str, Any]) -> ExecResult:
    ...
    sim = (params.get("sim") or {})  # resolved sim config dict
    threshold = float(params.get("unbound_threshold", 8.0))
    n_eq = int((sim.get("release_eq") and len(sim["release_eq"])) or 0)
    ...
    # Build trajectory list (skip the first run)
    trajs = [p["equil_dir"] / f"md-{i:02d}.nc" for i in range(1, n_eq)]
    trajs = [t for t in trajs if t.exists()]
    if not trajs:
        raise FileNotFoundError(f"[equil_check:{lig}] no equil trajectories found (md-01.nc..md-??.nc)")
    ...
    _cpptraj_export_rep(rep_idx, prmtop, n_eq, p["equil_dir"])
