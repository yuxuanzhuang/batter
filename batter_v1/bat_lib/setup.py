...
def sim_files(hmr, temperature, mol,
    ...
    if (stage == 'prep'):
        for i in range(0, num_sim):
            with open(f'../{amber_file_path}/mdin-prep', "rt") as fin:
                with open("./mdin-%03d" % int(i), "wt") as fout:
                    if i == 0:
                        for line in fin:
                            fout.write(line.replace('_temperature_', str(temperature)).replace(
                                '_num-atoms_', str(vac_atoms)).replace('_num-steps_', str(steps1)).replace('disang_file', 'disang%03d' % int(i)))
                    else:
                        for line in fin:
                            fout.write(line.replace('_temperature_', str(temperature)).replace(
                                '_num-atoms_', str(vac_atoms)).replace('_num-steps_', str(steps2)).replace('disang_file', 'disang%03d' % int(i)))
...
def dec_files(temperature, mol, num_sim,
              pose, comp, win, stage, steps1, steps2,
              weight, lambdas, dec_method, ntwx):
    """Create simulation files for decoupling simulations"""
