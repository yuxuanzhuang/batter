...
class MBARAnalysis(FEAnalysisBase):
    ...
    def _extract_all_for_window(
        ...
        logger.debug(
            f"[MBARAnalysis] {component}{win_i:02d} using {len(mdouts)} mdout files"
        )
        ...
        if df.empty:
            ...
            df = pd.concat(dfs)
            logger.warning(
                f"[MBARAnalysis] {component}{win_i:02d} WARNING: returning untruncated data!"
            )
        return df
    ...
    def plot_overlap_matrix(self, ax=None, **kwargs):
        return plot_mbar_overlap_matrix(
            self.results["convergence"]["overlap_matrix"], ax=ax, **kwargs
        )
    ...
    def plot_convergence(
        ...
        fig, axes = plt.subplot_mosaic(
            [["A", "A", "B"], ["C", "C", "C"]], figsize=(24, 14)
        )
        self.plot_time_convergence(
            ax=axes["A"], units=self.energy_unit, final_error=0.6
        )
        self.plot_overlap_matrix(ax=axes["B"])
        self.plot_block_convergence(
            ax=axes["C"], units=self.energy_unit, final_error=0.6
        )
...
class BoreschAnalysis(FEAnalysisBase):
    ...
    def run_analysis(self):
        ...
        with open(self.disangfile, "r") as f_in:
            ...
            fe_bd = self.fe_int(
                r0, a1_0, t1_0, a2_0, t2_0, t3_0, self.k_r, self.k_a, self.temperature
            )
...
def analyze_lig_task(
    ...
    try:
        ...
        fe_std = (
            float(np.sqrt(np.sum(np.array(fe_stds) ** 2))) if fe_stds else float("nan")
        )
    ...
    if rocklin_correction and "y" in components:
        ...
        if len(lig_ag) == 0:
            raise ValueError(
                f"No ligand atoms found for Rocklin correction with resname {mol}"
            )
        ...
        if lig_netq == 0:
            logger.debug(
                f"Rocklin correction skipped: ligand netq={lig_netq}, other netq={other_netq}"
            )
        if lig_netq != 0:
            logger.debug(
                f"Rocklin correction with ligand netq={lig_netq}, other netq={other_netq}"
            )
